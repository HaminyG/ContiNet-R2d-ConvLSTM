...
class keypoint:
    ...
    def get_x_y(self, keypoint):
        ...
        with open(self.path, 'r') as f:
            ...
            for num_frame in range(0, len(data), 1):
    ...
    def video_frame(self):
        ...
        for frame in range(0, self.framecount, 1):
            for left_hand_keypoint in (
            left[frame]):  # frame-1 means the first frame of input video is black, so we have to delete that frame
            ...
            for right_hand_keypoint in right[frame]:
                cv2.circle(self.signvideoframe[frame, :, :, :],
                           (round(right_hand_keypoint[0]), round(right_hand_keypoint[1])), 3, (255, 0, 0), -1)
            for body_keypoint in body[frame]:
            ...
            for index, pair in enumerate(self.hand_POSE_PAIRS):
                color = np.array(self.leftkeypint_color)
                color=color[index]
                partA = pair[0]
                partB = pair[1]

                if int(100*left[frame][partA][2]) and int(100*left[frame][partB][2])>10:
                    cv2.line(self.signvideoframe[frame, :, :,:], (int(left[frame][partA][0]),
                                                          int(left[frame][partA][1])),
                             (int(left[frame][partB][0]),
                              int(left[frame][partB][1])), (0,255,0), 2,
                             lineType=cv2.LINE_AA)
                else:
                    continue

            for index, pair in enumerate(self.hand_POSE_PAIRS):
                color = np.array(self.rightkeypoint_color)
                color=color[index]
                partA = pair[0]
                partB = pair[1]

                if int(100*right[frame][partA][2]) and int(100*right[frame][partB][2])>10:
                    cv2.line(self.signvideoframe[frame, :, :,:], (int(right[frame][partA][0]),
                                                          int(right[frame][partA][1])),
                             (int(right[frame][partB][0]),
                              int(right[frame][partB][1])),  (255,0,0), 2,
                             lineType=cv2.LINE_AA)
                else:
                    continue

            for index, pair in enumerate(self.body_POSE_PAIRS):
                color = np.array(self.bodykeypoint_color)
                color=color[index]
                partA = pair[0]
                partB = pair[1]

                if int(100*body[frame][partA][2]) and int(100*body[frame][partB][2])>0:
                    cv2.line(self.signvideoframe[frame, :, :,:], (int(body[frame][partA][0]),
                                                          int(body[frame][partA][1])),
                             (int(body[frame][partB][0]),
                              int(body[frame][partB][1])), (0,0,255), 2,
                             lineType=cv2.LINE_AA)
                else:
                    continue
